...
l22=8*x**2-10*x+3
l22x=16*x-10
trueMassBoundaryMat=np.array([[l10x[0]-params["PeM"], l11x[0], l12x[0], 0 ,0],
                              [0, 1, 0, 0, 0],
                              [l10x[2], l11x[2], l12x[2]-l20x[2],-l21x[2], -l22x[2]],
                              [0, 0, 0, 1, 0],
                              [0, 0, l20x[4], l21x[4], l22x[4]]
                              ])
trueTempBoundaryMat=np.array([[l10x[0]-params["PeT"], l11x[0], l12x[0], 0 , params["f"]*params["PeT"]],
                              [0, 1, 0, 0, 0],
                              [l10x[2], l11x[2], l12x[2]-l20x[2],-l21x[2], -l22x[2]],
                              [0, 0, 0, 1, 0],
                              [0, 0, l20x[4], l21x[4], l22x[4]]
...
params={"PeM": 1, "PeT": 1, "f":0, "Le": 1, "Da": .15, "beta": 1.3, "gamma": 10,"delta": 2, "vH":-.2}
...
dydt = model.dydt(y,0)
assert(np.isclose(dydt[:nPoints],-dudx+d2udx2/params["PeM"]+params["Da"]*(1-u)\
                            *np.exp(params["gamma"]*params["beta"]*v/(1+params["beta"]*v))).all())
assert(np.isclose(dydt[nPoints:],(-dvdx+d2vdx2/params["PeT"]+params["Da"]*(1-u)\
...
params={"PeM": 1000, "PeT": 100, "f":.3, "Le": 1, "Da": .15, "beta": 1.3, "gamma": 10,"delta": 2, "vH":-.2}
