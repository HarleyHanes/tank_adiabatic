...
podModes, podModesx, podModesxx, timeModes, podError = model.computePODmodes(np.eye(U.shape[0]),A,Ax,Axx,.9)
...
podModesWeighted, podModesInt, romMassMean, romFirstOrderMat, romFirstOrderMean, romSecondOrderMat, romSecondOrderMean= model.computeRomMatrices(W,mean,meanx,meanxx,podModes,podModesx,podModesxx)
...
uModesWeighted, uModesInt, uMassMean, uFirstOrderMat, uFirstOrderMean, uSecondOrderMat, uSecondOrderMean = model.computeRomMatrices(W, mean,meanx,meanxx,uModes,uModesx,uModesxx)
vModesWeighted, vModesInt, vMassMean, vFirstOrderMat, vFirstOrderMean, vSecondOrderMat, vSecondOrderMean = model.computeRomMatrices(W, mean,meanx,meanxx,vModes,vModesx,vModesxx)
...
romData = RomData(x, W, np.zeros(uModes.shape), mean, uModes, uModesx, uModesxx, uModesWeighted,
                        uModesInt, uMassMean, uFirstOrderMat, uFirstOrderMean,
                        uSecondOrderMat, uSecondOrderMean, np.zeros(uModes.shape), mean,
                        vModes, vModesx, vModesxx, vModesWeighted, vModesInt,
                        vMassMean, vFirstOrderMat, vFirstOrderMean,
                        vSecondOrderMat, vSecondOrderMean)
#Case 1: Linear cases
...
assert(np.isclose(dydt[2:4],(1/params["PeT"]*vSecondOrderMean-vFirstOrderMean
                            +params["delta"]*(params["vH"]*vModesInt-vMassMean))/params["Le"])).all()
...
modelCoeff=np.array([np.append(1+model.collocationPoints*0,1+model.collocationPoints*0), np.append(model.collocationPoints, 1/2*(3*model.collocationPoints**2-1))])
...
romData,truncationError = model.constructPodRom(modelCoeff,x,2,quadRule="simpson",mean="zero",useEnergyThreshold=False)
...
assert(np.isclose(romData.uRomFirstOrderMat,np.array([[0, 2],
                                           [0, 0]])).all())
assert(np.isclose(romData.uRomFirstOrderMean,np.array([2,0])).all())
assert(np.isclose(romData.uRomSecondOrderMat,np.array([[0, 0],
                                            [0, 0]])).all())
assert(np.isclose(romData.uRomSecondOrderMean,np.array([0,0])).all())

#assert(np.isclose(vMassMean,np.array([2,0])).all())
assert(np.isclose(romData.vRomFirstOrderMat,np.array([[0, 0],
                                           [0, 0]])).all())
assert(np.isclose(romData.vRomFirstOrderMean,np.array([2,0])).all())
assert(np.isclose(romData.vRomSecondOrderMat,np.array([[0, 6],
                                            [0, 0]])).all())
