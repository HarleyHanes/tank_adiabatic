...
u = lambda x,t: 1+np.sin(2*np.pi*(x+t))/2
v = lambda x,t: 1+np.cos(2*np.pi*(x+t))/2
...
romData, truncationError= model.constructPodRom(modelCoeff,x,W,1,mean="zero",nDeimPoints="none",useEnergyThreshold=False)
...
romData, truncationError= model.constructPodRom(modelCoeff,x,W,2,mean="zero",nDeimPoints=3,useEnergyThreshold=False)
print(romData.uNonLinProjection@romData.deimProjection-romData.uModesWeighted.transpose())
assert(np.isclose(romData.uNonLinProjection@romData.deimProjection,romData.uModesWeighted.transpose(),atol=1e-07).all())
assert(np.isclose(romData.vNonLinProjection@romData.deimProjection,romData.vModesWeighted.transpose(),atol=1e-07).all())

#Case 2: mean, uniform points
x,W= model.getQuadWeights(3,"uniform")

romData, truncationError= model.constructPodRom(modelCoeff,x,W,2,mean="zero",nDeimPoints="none",useEnergyThreshold=False)
assert(np.isclose(romData.uNonLinProjection,romData.uModesWeighted.transpose()).all())
assert(np.isclose(romData.vNonLinProjection,romData.vModesWeighted.transpose()).all())
romData, truncationError= model.constructPodRom(modelCoeff,x,W,1,mean="zero",nDeimPoints=3,useEnergyThreshold=False)
print(romData.uNonLinProjection@romData.deimProjection-romData.uModesWeighted.transpose())
assert(np.isclose(romData.uNonLinProjection@romData.deimProjection,romData.uModesWeighted.transpose(),atol=1e-07).all())
assert(np.isclose(romData.vNonLinProjection@romData.deimProjection,romData.vModesWeighted.transpose(),atol=1e-07).all())
...
romData, truncationError= model.constructPodRom(modelCoeff,x,W,1,mean="mean",nDeimPoints="none",useEnergyThreshold=False)
assert(np.isclose(romData.uNonLinProjection,romData.uModesWeighted.transpose()).all())
assert(np.isclose(romData.vNonLinProjection,romData.vModesWeighted.transpose()).all())
romData, truncationError= model.constructPodRom(modelCoeff,x,W,1,mean="mean",nDeimPoints=3, useEnergyThreshold=False)
assert(np.isclose(romData.uNonLinProjection@romData.deimProjection,romData.uModesWeighted.transpose(),atol=1e-07).all())
assert(np.isclose(romData.vNonLinProjection@romData.deimProjection,romData.vModesWeighted.transpose(),atol=1e-07).all())
