...
class TankModel:
    ...
    def nElements(self, value):
        if isinstance(value, np.ndarray):
            ...
            elif not isinstance(value[0], np.integer):
        ...
        if not isinstance(value, list):
            ...
            for iElement in range(len(value)):
                if not isinstance(value[iElement], Element):
                    raise Exception(
                        "Non-Element object entered for elements[" + str(iElement) + "]: " + str(value[iElement])
                    )
    ...
    def firstOrderMat(self, value):
        if not isinstance(value, np.ndarray):
            raise Exception("Non-numpy array entered for firstOrderMat: " + str(value))
        elif value.shape != (
            self.nElements * self.nCollocation,
            self.nElements * (self.nCollocation + 1) + 1,
        ):
            raise Exception(
                "Matrix of incorrect size entered for firstOrderMat: firstOrderMat.size=" + str(value.shape)
            )
        else:
    ...
    def secondOrderMat(self, value):
        if not isinstance(value, np.ndarray):
            raise Exception("Non-numpy array entered for secondOrderMat: " + str(value))
        elif value.shape != (
            self.nElements * self.nCollocation,
            self.nElements * (self.nCollocation + 1) + 1,
        ):
            raise Exception(
                "Matrix of incorrect size entered for secondOrderMat: secondOrderMat.size=" + str(value.shape)
            )
        else:
    ...
    def massBoundaryMat(self, value):
        if not isinstance(value, np.ndarray):
            ...
            raise Exception(
                "Matrix of incorrect size entered for massBoundaryMatInv: massBoundaryMat.size=" + str(value.shape)
            )
    ...
    def tempBoundaryMat(self, value):
        if not isinstance(value, np.ndarray):
            ...
            raise Exception(
                "Matrix of incorrect size entered for tempBoundaryMat: tempBoundaryMat.size=" + str(value.shape)
            )
    ...
    def tempBoundaryMatInv(self, value):
        if not isinstance(value, np.ndarray):
            ...
            raise Exception(
                "Matrix of incorrect size entered for tempBoundaryMatInv: tempBoundaryMat.size=" + str(value.shape)
            )
    ...
    def params(self, value):
        ...
        elif not isinstance(value["gamma"], (int, float, complex)):
    ...
    def copy(self, **overrides):
        ...
        current_args = {
            ...
            "bounds": (
                ...
                else (self.bounds.copy() if isinstance(self.bounds, np.ndarray) else self.bounds)
            ),
            "spacing": self.spacing,
            "params": self.params.copy() if isinstance(self.params, dict) else dict(self.params),
            "tEval": (
                self.tEval.copy()
                if isinstance(self.tEval, np.ndarray)
    ...
    def __computeCollocationMatrices__(self):
        ...
        for iRow in range(self.nCollocation * self.nElements):
            ...
            for iCol in np.arange(element * (self.nCollocation + 1), (element + 1) * (self.nCollocation + 2) - element):
                ...
                firstOrderMat[iRow, iCol] = self.elements[element].basisFirstDeriv(
                    self.elements[element].collocationPoints[collocation]
                )[basis]
                secondOrderMat[iRow, iCol] = self.elements[element].basisSecondDeriv(
                    self.elements[element].collocationPoints[collocation]
                )[basis]
        ...
        for iRow in np.arange(0, self.nElements * (self.nCollocation + 1) + 1, self.nCollocation + 1):
            ...
            # Internal Boundaries
                ...
                tempBoundaryMat[
                    iRow,
                    ((element - 1) * (self.nCollocation + 1)) : (element * (self.nCollocation + 1) + 1),
                ] = self.elements[element - 1].basisFirstDeriv(self.elements[element].interpolationPoints[0])
                tempBoundaryMat[
                    iRow,
                    (element * (self.nCollocation + 1)) : ((element + 1) * (self.nCollocation + 1) + 1),
        ...
        for i in range(self.nElements):
            ...
            pointExpansionMat[rowStart : rowStart + self.nCollocation, colStart : colStart + self.nCollocation] = (
                np.eye(self.nCollocation)
            )
        ...
        if self.verbosity > 1:
            ...
            print(
                "Mass Full Domain Condition Number: "
                + str(np.linalg.cond(-self.firstOrderMat + 1 / self.params["PeM"] * self.secondOrderMat))
            )
            print(
                "Temp Full Domain Condition Number: "
                + str(
                    np.linalg.cond(
                        (-self.firstOrderMat + 1 / self.params["PeT"] * self.secondOrderMat) / self.params["Le"]
                    )
                )
            )
            if self.verbosity > 2:
        ...
        self.massRHSmat = np.matmul(
            -self.firstOrderMat + 1 / self.params["PeM"] * self.secondOrderMat,
            self.massFullCoeffMat,
        )
        self.tempRHSmat = np.matmul(
            -self.firstOrderMat + 1 / self.params["PeT"] * self.secondOrderMat,
            self.tempFullCoeffMat,
        )
        if self.verbosity > 1:
        ...
        doublePointExpansionMat = np.append(
            doublePointExpansionMat,
            np.append(np.zeros(pointExpansionMat.shape), pointExpansionMat, axis=1),
            axis=0,
        )
        dudPeMboundaryMat = np.zeros(
            (
                2 * ((self.nCollocation + 1) * self.nElements + 1),
                2 * ((self.nCollocation + 1) * self.nElements + 1),
            )
        )
        dudPeMboundaryMat[
            : (self.nCollocation + 1) * self.nElements + 1,
            : (self.nCollocation + 1) * self.nElements + 1,
        ] = self.massBoundaryMat
        dudPeMboundaryMat[
            (self.nCollocation + 1) * self.nElements + 1 :,
            (self.nCollocation + 1) * self.nElements + 1 :,
        ] = self.massBoundaryMat
        ...
        dudPeMfullCoeffMat = np.linalg.solve(dudPeMboundaryMat, doublePointExpansionMat)[
            (self.nCollocation + 1) * self.nElements + 1 :,
        ]
        self.dudPeMrhsMat = np.matmul(
            -self.firstOrderMat + 1 / self.params["PeM"] * self.secondOrderMat, dudPeMfullCoeffMat
        )
        self.dudPeMSecondOrderMat = np.matmul(self.secondOrderMat / (self.params["PeM"] ** 2), self.massFullCoeffMat)

        dvdPeTboundaryMat = np.zeros(
            (
                2 * ((self.nCollocation + 1) * self.nElements + 1),
                2 * ((self.nCollocation + 1) * self.nElements + 1),
            )
        ...
        dvdPeTboundaryMat[
            : (self.nCollocation + 1) * self.nElements + 1,
            : (self.nCollocation + 1) * self.nElements + 1,
        ] = self.tempBoundaryMat
        dvdPeTboundaryMat[
            (self.nCollocation + 1) * self.nElements + 1 :,
            (self.nCollocation + 1) * self.nElements + 1 :,
        ] = self.tempBoundaryMat
        dvdPeTboundaryMat[(self.nCollocation + 1) * self.nElements + 1, 0] = -1
        dvdPeTboundaryMat[(self.nCollocation + 1) * self.nElements + 1, (self.nCollocation + 1) * self.nElements] = (
            self.params["f"]
        )
        self.dvdPeTboundaryMat = dvdPeTboundaryMat
        dvdPeTfullCoeffMat = np.linalg.solve(dvdPeTboundaryMat, doublePointExpansionMat)[
            (self.nCollocation + 1) * self.nElements + 1 :,
        ]
        self.dvdPeTrhsMat = np.matmul(
            -self.firstOrderMat + 1 / self.params["PeT"] * self.secondOrderMat, dvdPeTfullCoeffMat
        )
        self.dvdPeTSecondOrderMat = np.matmul(self.secondOrderMat / (self.params["PeT"] ** 2), self.tempFullCoeffMat)

        dvdfboundaryMat = np.zeros(
            (
                2 * ((self.nCollocation + 1) * self.nElements + 1),
                2 * ((self.nCollocation + 1) * self.nElements + 1),
            )
        ...
        dvdfboundaryMat[(self.nCollocation + 1) * self.nElements + 1, (self.nCollocation + 1) * self.nElements] = (
            self.params["PeT"]
        )
        self.dvdfboundaryMat = dvdfboundaryMat
        dvdffullCoeffMat = np.linalg.solve(dvdfboundaryMat, doublePointExpansionMat)[
            (self.nCollocation + 1) * self.nElements + 1 :,
        ]
        self.dvdfrhsMat = np.matmul(
            -self.firstOrderMat + 1 / self.params["PeT"] * self.secondOrderMat, dvdffullCoeffMat
        )
    ...
    def dydt(self, y, t):
        ...
        # Original (expanded) formulation kept for reference:
        # dydt = [ M*u + Da*(1-u)*exp(gamma*beta*v/(1+beta*v)),
        #          T*v + ( Da*(1-u)*exp(gamma*beta*v/(1+beta*v))
        #                 + delta*(vH - v) ) / Le ]
        dydt = np.append(
            np.dot(self.massRHSmat, u)
            + self.params["Da"]
            * (1 - u)
            * np.exp(self.params["gamma"] * self.params["beta"] * v / (1 + self.params["beta"] * v)),
            (
                np.dot(self.tempRHSmat, v)
                + self.params["Da"]
                * (1 - u)
                * np.exp(self.params["gamma"] * self.params["beta"] * v / (1 + self.params["beta"] * v))
                + self.params["delta"] * (self.params["vH"] - v)
            )
            / self.params["Le"],
        )
    ...
    def dydtSens(self, y, t, paramSelect=[]):
        ...
        for param in paramSelect:
            ...
            match param:
                case "Da":
                    nonLinearTerm = np.exp(
                        self.params["gamma"] * self.params["beta"] * v / (1 + self.params["beta"] * v)
                    ) * (
                        1
                        - u
                        + self.params["Da"]
                ...
                case "gamma":
                    nonLinearTerm = (
                        ...
                        * (
                            ...
                            + (1 - u)
                            * self.params["beta"]
                            * (v + self.params["beta"] * (v**2) + self.params["gamma"] * dvdParam)
                            / (1 + self.params["beta"] * v) ** 2
                        )
                    )
                case _:
    ...
    def eval(self, xEval, modelCoeff, output="full", deriv=0):
        ...
        ----------
        xEval : array-like
            Spatial locations where solution is evaluated.
        modelCoeff : ndarray
            Coefficients (interior only) or snapshots.
        output : {"full","seperated","u","v"}
            Controls returned arrays.
        deriv : int, optional
            0=value, 1=first deriv, 2=second deriv.
        """
        if output not in ("full", "seperated", "u", "v"):
            raise (Exception("Invalid entry for output used, must be full, seperated, u, or v"))
        # Ensure xEval is an ndarray so logical ops work on indices
        if not isinstance(xEval, np.ndarray):
        ...
        for iElement in range(self.nElements):
            ...
            # Get x values for interior points of selected element.
            # (Marker value choice 0.25–0.5 acceptable; using 0.4 here.)
            ...
            if modelCoeff.ndim == 1:
                uEval[xElementIndices] = np.dot(
                    uFull[iElement * (self.nCollocation + 1) : (iElement + 1) * (self.nCollocation + 1) + 1],
                    basisValues,
                )
                vEval[xElementIndices] = np.dot(
                    vFull[iElement * (self.nCollocation + 1) : (iElement + 1) * (self.nCollocation + 1) + 1],
                    basisValues,
                )
            # No need to check uFull/vFull dims here;
            # computeFullCoeff validates 1D/2D cases.
            else:
                # print("uFull for elem", iElement)
                # print("basisValues for elem", iElement)
                uEval[:, xElementIndices] = np.dot(
                    uFull[
                        :,
                        iElement * (self.nCollocation + 1) : (iElement + 1) * (self.nCollocation + 1) + 1,
                    ],
                    basisValues,
                )
                vEval[:, xElementIndices] = np.dot(
            ...
            # print("Computed uEval values for element", iElement)
            # print("uEval after element", iElement)
        if output == "seperated":
            return uEval, vEval
        elif output == "u":
            return uEval
    ...
    def computeFullCoeff(self, collocationCoeff, output="full"):
        """Compute full (interior + boundary) coefficients for each element.

        Applies closure relations encoded in *FullCoeffMat.
        """
    ...
    def constructPodRom(
        ...
        vEvalxx -= vMeanxx
        if np.isclose(
            modelCoeff[..., : self.nElements * self.nCollocation],
            modelCoeff[..., self.nElements * self.nCollocation :],
            atol=1e-10,
        ).all():
        ...
        uModes, uModesx, uModesxx, uTimeModes, uTruncationError, uSingularValues, uFullSpectra = self.computePODmodes(
            W,
            uEval.transpose(),
            uEvalx.transpose(),
            uEvalxx.transpose(),
            ...
            quadPrecision=quadPrecision,
        )
        vModes, vModesx, vModesxx, vTimeModes, vTruncationError, vSingularValues, vFullSpectra = self.computePODmodes(
            W,
            vEval.transpose(),
            vEvalx.transpose(),
            vEvalxx.transpose(),
        ...
        if self.bounds[0] == x[0]:
            for i in range(uModes.shape[1]):
                if not np.isclose(uModes[0, i] - uModesx[0, i] / self.params["PeM-boundary"], 0, atol=1e-6):
                    print(
                        "Left Boundary Condition not satisfied for uMode",
                        i,
                        "by: ",
                        uModes[0, i] - uModesx[0, i] / self.params["PeM"],
                    )
                if not np.isclose(uModesx[-1, i], 0, atol=1e-6):
                    print(
                        "Right Boundary Condition not satisfied for uMode",
                        i,
                        "by: ",
                        uModesx[-1, i],
                    )
            for i in range(vModes.shape[1]):
                if not np.isclose(
                    vModes[0, i] - vModesx[0, i] / self.params["PeT-boundary"] - vModes[-1, i] * self.params["f"],
                    0,
                    atol=1e-6,
                ):
                    print(
                        "Left Boundary Condition not satisfied for vMode",
                        i,
                        "by: ",
                        vModes[0, i] - vModesx[0, i] / self.params["PeM"] - vModes[-1, i] * self.params["f"],
                    )
                if not np.isclose(vModesx[-1, i], 0, atol=1e-6):
                    print(
                        "Right Boundary Condition not satisfied for vMode",
                        i,
                        "by: ",
                        vModesx[-1, i],
                    )
    ...
    def computeNonLinReduction(self, romData, nonlinDim, proportionality="mode number"):
        if nonlinDim == "max":
            ...
            if nonlinDim <= 1:
                ...
                raise Exception("Nonlinear dim > 1 with 'mode number' proportionality; must be <= 1.")
            ...
            if nonlinDim <= 1:
                ...
                raise Exception("Nonlinear dim > 1 with 'singular value' proportionality; must be <= 1.")
            ...
            if nonlinDim >= 1:
                ...
                uNonlinDim = max(
                    uPropInformation.size - int(np.sum(uPropInformation < (uTruncation * nonlinDim))),
                    1,
                )
                vNonlinDim = max(
                    vPropInformation.size - int(np.sum(vPropInformation < (vTruncation * nonlinDim))),
                    1,
                )
            else:
                raise Exception("Nonlinear dim > 1 with 'singular value' proportionality; must be <= 1.")
            ...
            if nonlinDim >= 1:
                # Use 12-digit rounding to avoid machine precision edge cases.
                uNonlinDim = min(
                    int(
                        np.sum(
                            np.floor(romData.uSingularValues * 10e12) / 10e12
                            > (romData.uSingularValues[-1] * nonlinDim)
                        )
                    )
                    + 1,
                    romData.uModes.shape[1],
                )
                vNonlinDim = min(
                    int(
                        np.sum(
                            np.floor(romData.vSingularValues * 10e12) / 10e12
                            > (romData.vSingularValues[-1] * nonlinDim)
                        )
                    )
                    + 1,
                    romData.vModes.shape[1],
                )
            else:
                raise Exception("Nonlinear dim < 1 with 'singular value' proportionality; must be >= 1.")
        else:
            raise Exception(f"Invalid proportionality '{proportionality}'. Use 'mode number' or 'singular value'.")
    ...
    def computeDEIMProjection(self, romData, nDeimPoints):
        ...
        nDeimPoints = min(
            int(np.ceil(nDeimPoints * max(romData.uModes.shape[1], romData.vModes.shape[1]))),
            romData.uModes.shape[0],
        )
    ...
    def computeDEIMmatrices(self, podBasisWeighted, deimBasis, P):
        # Solve A from (PhiW)' Psi (P' Psi)^{-1}; rearranged to (Psi' P) A' = Psi'(PhiW)
        deimProjection = np.linalg.solve(
            deimBasis.transpose() @ P, deimBasis.transpose() @ podBasisWeighted
        ).transpose()
        # Note: Projection is computed each step for simplicity.
        # To optimize, precompute and pass unweighted POD modes.
    ...
    def computePODmodes(
        ...
        useEnergyThreshold=True,
        adjustModePairs=False,
        groupSeperations="null",
        quadPrecision=False,
    ):
        # TODO: Optionally scale observed responses to [0,1] so large magnitudes don't dominate POD.
        ...
        if np.isclose(W / W[0, 0], np.eye(W.shape[0])).all():
            ...
            if quadPrecision:
                ...
                #         "Error: ",
                #         np.sqrt(
                #             np.sum(np.sum((timeModes @ S - S @ null.transpose()) ** 2))
                #             / np.sum(np.sum((timeModes @ S) ** 2))
                #         ),
                #     )
                #     if self.verbosity >=3:
                #         print("W: ", W)
        ...
        if adjustModePairs and nModes > 1:
            ...
            adjacent_distance = np.abs(
                np.array(
                    [
                        modes[:, nModes - 1].transpose() @ W @ modesx[:, nModes - 2],
                        modes[:, nModes - 1].transpose() @ W @ modesx[:, nModes],
                    ]
                )
            )
        ...
        if not np.isclose(modes.transpose() @ W @ modes, np.eye(modes.shape[1])).all():
            ...
            if self.verbosity >= 3:
                ...
                print(
                    "Departure from Orthonormality: ",
                    np.sum(np.eye(modes.shape[1]) - modes.transpose() @ W @ modes),
                )
    ...
    def getQuadWeights(self, nPoints, quadRule):
        if quadRule == "simpson":
            ...
            x = np.append(
                x,
                np.sort(np.random.sample(nPoints - 2)) * (self.bounds[1] - self.bounds[0]) + self.bounds[0],
                axis=0,
            )
    ...
    def dydtPodRom(self, y, t, romData, paramSelect=[]):
        if isinstance(paramSelect, str):
            paramSelect = [paramSelect]
        u = y[0 : romData.uNmodes]
        v = y[romData.uNmodes : romData.uNmodes + romData.vNmodes]
        # NOTE: This is not optimal (recomputes projections) but simplifies DEIM on/off.
        # Optimal: pre-compute P^T @ modes and P^T @ mean for u and v.
        uNonlin = romData.deimProjection.transpose() @ (
            romData.uModes[:, : romData.uNonlinDim] @ u[: romData.uNonlinDim] + romData.uMean
        )
        vNonlin = romData.deimProjection.transpose() @ (
            romData.vModes[:, : romData.vNonlinDim] @ v[: romData.vNonlinDim] + romData.vMean
        )
        ...
        # dvdt: nonlinear + boundary penalty; optimized form below
        dudt = (
            (romData.uRomSecondOrderMat / self.params["PeM"] - romData.uRomFirstOrderMat) @ u
            + romData.uRomSecondOrderMean / self.params["PeM"]
            - romData.uRomFirstOrderMean
            + self.params["Da"]
            * romData.uNonLinProjection
            @ (
                (1 - uNonlin)
        ...
        dvdt = (
            (romData.vRomSecondOrderMat / self.params["PeT"] - romData.vRomFirstOrderMat) @ v
        ...
        for param in paramSelect:
            dudParam = y[
                eqCounter * (romData.uNmodes + romData.vNmodes) : eqCounter * (romData.uNmodes + romData.vNmodes)
                + romData.uNmodes
            ]
            dvdParam = y[
