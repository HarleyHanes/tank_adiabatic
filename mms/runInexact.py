...
error, solutions, convergence, errorSpace, convergenceSpace = tankMMS.runMMStest(
    higherOrderTerms, nCollocations, nElems, xEval, tEval, params, verbosity=verbosity
)
...
for iOrder in range(len(higherOrderTerms)):
    ...
    for iColl in range(len(nCollocations)):
        ...
        convergenceExampleStartValue = np.min(errorSpace[iColl, 0, -1, iOrder, :, :, 0]) * 0.35
        ...
        nElems_example = nElems[convergenceExample >= (np.min(errorSpace[iColl, :, -1, iOrder, :, :, 0])) * 0.1]
        convergenceExample = convergenceExample[
            convergenceExample >= (np.min(errorSpace[iColl, :, -1, iOrder, :, :, 0])) * 0.1
        ]
        plt.figure(figsize=(5.12, 3.84))
        ...
        plt.legend(
            [
                ...
                r"v, $L^2$ Error",
                r"v, $L^\inf$ Error",
                r"$\mathcal{O}((1/N_E)^" + str(expectedRate) + r")$",
            ]
        )
        ...
        # Get an example of the expected convergence rate of the spatial discretization
        expectedRate = nCollocations[iColl] + 2
        convergenceExampleStartValue = np.min(errorSpace[iColl, 0, -1, iOrder, :, :, -1]) * 0.35
        convergenceExample = convergenceExampleStartValue * (nElems[0] / nElems) ** (expectedRate)
        # Trim the expected convergence line to the minimum of the observed error
        nElems_example = nElems[convergenceExample >= (np.min(errorSpace[iColl, :, -1, iOrder, :, :, -1])) * 0.1]
        convergenceExample = convergenceExample[
            convergenceExample >= (np.min(errorSpace[iColl, :, -1, iOrder, :, :, -1])) * 0.1
        ]
        ...
        for itemporal in range(3):
            expectedRate = nCollocations[iColl] + 2
            convergenceExampleStartValue = np.min(error[iColl, 0, itemporal, iOrder, :, :]) * 0.35
            convergenceExample = convergenceExampleStartValue * (nElems[0] / nElems) ** (expectedRate)
            nElems_example = nElems[convergenceExample >= (np.min(error[iColl, :, itemporal, iOrder, :, :]) * 0.1)]
            convergenceExample = convergenceExample[
                convergenceExample >= (np.min(error[iColl, :, itemporal, iOrder, :, :]) * 0.1)
            ]
        ...
        plt.plot(
            xEval,
            solutions[iColl, 0, 0, iOrder, 0, 0, 0, :] - solutions[iColl, 0, 0, iOrder, 0, 1, 0, :],
        )
        plt.plot(
            xEval,
            solutions[iColl, 0, 0, iOrder, 1, 0, 0, :] - solutions[iColl, 0, 0, iOrder, 1, 1, 0, :],
        )
        plt.legend(["u", "v"])
