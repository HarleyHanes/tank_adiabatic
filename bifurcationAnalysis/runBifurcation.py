    ...
    for iy0 in range(y0.shape[0]):
        for r in np.arange(paramMin, paramMax, paramStep):
            ...
            for it in range(max_iterations-1):
                odeOut= scipy.integrate.solve_ivp(lambda t,y: model.dydt(y,t),(tEval[it],tEval[it+1]),modelCoeff, method='BDF',atol=1e-13,rtol=1e-13)
...
plotFromData=True
if not plotFromData:
    baseParams={"PeM": 300, "PeT": 300, "f": .3, "Le": 1, "Da": .15, "beta": 1.4, "gamma": 10,"delta": 2, "vH":-.03}
    ...
    for it in range(tEval.size-1):
        odeOut= scipy.integrate.solve_ivp(lambda t,y: model.dydt(y,t),(tEval[it],tEval[it+1]),modelCoeff[0], method='BDF',atol=1e-13,rtol=1e-13)
        modelCoeff[0] = odeOut.y[:,-1]

    baseParams={"PeM": 300, "PeT": 300, "f": .3, "Le": 1, "Da": .15, "beta": 1.4, "gamma": 10,"delta": 2, "vH":-.09}
    model=TankModel(nCollocation=2,nElements=64,spacing="legendre",bounds=[0,1],params=baseParams)
    tEval=np.linspace(0,200,26)
    for it in range(tEval.size-1):
        odeOut= scipy.integrate.solve_ivp(lambda t,y: model.dydt(y,t),(tEval[it],tEval[it+1]),modelCoeff[1], method='BDF',atol=1e-13,rtol=1e-13)
...
plt.xlabel(r"$v_H$")
plt.ylabel(r"$v(t,1)$")
ax.set_xticks([-.09, -.07, -.05, -.03])
ax.set_yticks([-.05, .05, .15, .25, .35, .45])
plt.tight_layout() 
