...
class Element:
    ...
    def interpolationPoints(self, value):
        if not isinstance(value, (np.ndarray, list)):
            ...
            if value.size != self.nCollocation + 2 or value.ndim != 1:
                raise Exception(
                    "Numpy array of non 1D with size order+2 entered for interpolationPoints: " + str(value)
                )
            else:
    ...
    def __setCollocationPoints__(self):
        if self.spacing == "uniform":
            self.collocationPoints = np.linspace(self.bounds[0], self.bounds[1], num=self.nCollocation + 2)[
                1 : self.nCollocation + 1
            ]
    ...
    def integrate(self, f, order="auto"):
        ...
        if self.spacing == "uniform":
            ...
            points = (np.polynomial.legendre.leggauss(order)[0] + 1) * (
